...
ajaxSuggestFbml.prototype.draw_results = function(results, typed) {
	...
	for( var i = 0; i < results.length; i++ ) {
		...
		var em_item = document.createElement('em').setTextValue(results[i].substring(0, typed.length));
		item.appendChild(em_item);

		var span_item = document.createElement('span').setTextValue(results[i].substring(typed.length));
