...
function adjust() {
    var outerWidth = windowWidth() - 20;
    $('#container').width( outerWidth );
    $('#container').css("left", 50);
    if (journeys.length>0 ) {
	$('#outerDiv').width( outerWidth);
	$('#outerDiv').height( windowHeight() - 160);   
	$('#outerDiv').css("left", 50);
	$('#innerDiv').height( windowHeight() - 175);
	$('#innerDiv').width( outerWidth);
	$('#innerDiv').css('left', 50);
    }
}
...
function loadtrains(date) {
    $('#loadgif').image('/images/ajax-loader.gif', function () {
	    $.get('/trainjourneys' + stations + date, {} , function (data) {
		    $('#trains').append(data);
			...
			js[i].id = js[i].id.replace(/other/,'');
			journeys[numberOfJourneys] = js[i];
			numberOfJourneys++;
		    }
...
function scrollJourneys(slideTo){
	...
	if (journeys[previousid].starttime >= slideTo) {
	    scrollToTrain(previousid, true);
	}
