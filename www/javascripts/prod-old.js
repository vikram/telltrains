...
function gm_setMap(params, title) {
...
function gm_addMarker(point, title){
	if(gm_marker!=null){gmMapObject.removeOverlay(gm_marker);}
	gm_marker = new GMarker(point);
	GEvent.addListener(gm_marker, "click",
			   function() {
			       gm_marker.openInfoWindowHtml(title);});
...
function adjust() {
    ...
    $('outerDiv').style.width = windowWidth() - 50 + 'px';
    $('outerDiv').style.height = windowHeight() - 168 + 'px';   
    $('innerDiv').style.height = windowHeight() - 182 + 'px';
    $('innerDiv').style.width = windowWidth() - 50 + 'px';
}

function scrollTrains() {

    var pos = Math.round($('innerDiv').scrollTop / 39.19);
	...
	var previous = $(journeys[index].id);
	
	sliderValues = [journeys[index].starttime];
	slider.setValue(sliderValues[0]);
	
	var sometop = stripPx(previous.style.top);
	var left = 0;
	if (sometop != 1) {
	    left = stripPx($(journeys[0].id).style.left) - stripPx(previous.style.left);
	}
	
	$('timeline').style.left = originalleft + left + 'px';
	$('lines').style.left = 180 + left + 'px';
	$('now').style.left = left + 'px';
	$('markers').style.left = 180 + left +  'px';
...
function init() {
    ...
    //dragThis(document.getElementById("now"));
    //showhideOptions();
    //filterJourneys();
    //if (window.addEventListener) {
    //window.addEventListener('DOMMouseScroll', wheel, false);
    //}
    //window.onmousewheel = document.onmousewheel = $('outerDiv').onmousewheel = wheel;
    //window.onscroll = document.onscroll = scrollTrains;

    outerDivTop = stripPx($('outerDiv').style.top);
    adjust();
    scrollJourneys(sliderValues);

    $('innerDiv').onscroll = scrollTrains;
}
...
function startMove(event) {
    ...
    ntop = stripPx(document.getElementById("now").style.top);
    dragging = true;
    return false;
}
function processMove(event) {
	...
	now.style.top = ntop + (event.clientY - dragStartTop) + 'px';
	lines.style.top = ltop + (event.clientY - dragStartTop) + 'px';
	innerDiv.style.top = top + (event.clientY - dragStartTop) + 'px';
    }
}

function handle(delta) {
    sliderValues = [sliderValues[0] - delta * 3];
    slider.setValue(sliderValues[0]);
    scrollJourneys(sliderValues[0]);
}
...
function handle2(delta) {
    ...
    var now = document.getElementById("now");
    if (delta < 0) {
	lines.style.top = stripPx(lines.style.top) - (-50 * delta) + 'px';
	innerDiv.style.top = stripPx(innerDiv.style.top) - (-50 * delta) + 'px';
	now.style.top = stripPx(now.style.top) - (-50 * delta) + 'px';
    }
    else {
	lines.style.top = stripPx(lines.style.top) + (50 * delta) + 'px';
	innerDiv.style.top = stripPx(innerDiv.style.top) + (50 * delta) + 'px';
...
function filterJourneys(){
	...
	var journey = $(journeys[i].id);
	if (shouldFilter(journeys[i])) {
	    if (journey.style.opacity != "0.2") {
		new Effect.Fade(journey, {to:0.2});
		doneSomething = true;
	    }
	} else if (journey.style.opacity == "0.2") {
	    new Effect.Appear (journey);
	    doneSomething = true;
	}
    }
    if (doneSomething) {
	new Effect.Highlight('timeline');
	new Effect.Highlight('now');
    }
}
...
function scrollJourneys(slideTo){
	...
	var previous = $(journeys[0].id);

	for (var i=0; i< numberOfJourneys; i++) {
	    if (journeys[i].starttime >= slideTo) {
		//hopefully the first one
		var sometop = stripPx(previous.style.top);
		var top = '0px';
		var left = 0;
		if (sometop != 1) {
		    top = -39.5 * ((sometop - 1) / 3) + 'px';
		    left = stripPx($(journeys[0].id).style.left) - stripPx(previous.style.left);
		}
		innerDiv.style.top = top;
		lines.style.top = top;
		now.style.top = top;
		innerDiv.style.left = originalleft + left + 'px';
		lines.style.left = 180 + left + 'px';
		markers.style.left = 180 + left +  'px';
		now.style.left = left + 'px';
		break;
...
function showhideOptions() {
    if ($('routeli')) {
	new Effect.toggle('routeli','appear',{duration:0.0}); 
	new Effect.toggle('changeli','appear',{duration:0.0}); 
    }
}
